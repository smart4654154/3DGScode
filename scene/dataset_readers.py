...
# 计算这些相机的平均中心的负值和半径大小
    ...
    def get_center_and_diag(cam_centers):
        ...
        dist = np.linalg.norm(cam_centers - center, axis=0, keepdims=True)#np.linalg.norm函数计算平方开根号
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        sys.stdout.flush()
        #它通过调用sys.stdout.write()函数来输出当前读取的相机序号和总相机数，并使用'\r'实现光标回到行首，实现动态更新进度。
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")#外参
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")#内参
    ...
    # 相机位置和半径  #半径=1.1*  max(相机位置与平均相机位置的距离)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    '''
    用于readNerfSyntheticInfo读取合成数据blender
    '''
    cam_infos = []
