...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(  # 用于定义前向渲染的规则，接受一系列输入参数，并调用 C++/CUDA 实现的 _C.rasterize_gaussians 方法进行高斯光栅化。
        ...
        # Restructure arguments the way that the C++ lib expects them#按照C++库所期望的方式重新构造参数
        args = (
            raster_settings.bg,
            means3D,
            colors_precomp,
        ...
        # Invoke C++/CUDA rasterizer#调用C++/CUDA光栅化器
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted#在它们被破坏之前复制它们
            try:
                num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(
                    *args)  # C++/CUDA 光栅化计算的输出结果。
        ...
        # Keep relevant tensors for backward#保留相关张量以备后发
        ...
        return color, radii#color就是渲染后的图像
